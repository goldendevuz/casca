...
act-env:
	. env/bin/activate
i:
	pip install --upgrade pip && pip install -r requirements/development.txt
mig:
...
run:
	python manage.py runserver 0.0.0.0:1028
...
run-wsgi:
	gunicorn config.wsgi:application --bind 0.0.0.0:1028
tunnel:
